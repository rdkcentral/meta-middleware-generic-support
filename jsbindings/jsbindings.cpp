...
static JSValueRef AAMP_getProperty_AudioLanguage(JSContextRef context, JSObjectRef thisObject, JSStringRef propertyName, JSValueRef* exception)
{
	...
	std::string language = pAAMP->_aamp->GetAudioLanguage();
...
static JSValueRef AAMP_getProperty_CurrentDRM(JSContextRef context, JSObjectRef thisObject, JSStringRef propertyName, JSValueRef* exception)
{
	...
	std::string drm = pAAMP->_aamp->GetDRM();
	LOG_INFO(pAAMP,"_aamp->GetDRM() %s",drm.c_str());
...
public:
		...
		LOG_TRACE("AAMP_JSListener_DRMMetadata NetworkMetric %s", networkMetric);
		...
		name = JSStringCreateWithUTF8CString("networkMetrics");
		JSObjectSetProperty(context, eventObj, name, aamp_CStringToJSValue(context, networkMetric), kJSPropertyAttributeReadOnly, NULL);
		JSStringRelease(name);
		...
		if(!headerVec.empty())
		{
			...
			{
				JSValueRef respHeader = aamp_CStringToJSValue(context, headerVec[i].c_str());
				array[i] = respHeader;
			}
			JSValueRef prop = JSObjectMakeArray(context, count, array, NULL);
			SAFE_DELETE_ARRAY(array);
			name = JSStringCreateWithUTF8CString("headers");
		...
		{
			JSObjectRef bodyResponseObj = aamp_CreateBodyResponseJSObject(context, pBodyResponse);
			if( bodyResponseObj )
			{
				JSValueProtect(context, bodyResponseObj);
				name = JSStringCreateWithUTF8CString("body");
				JSObjectSetProperty(context, eventObj, name, bodyResponseObj, kJSPropertyAttributeReadOnly, NULL);
				JSStringRelease(name);
...
/**
 * @class AAMP_JSListener_ManifestFileRefresh
 * @brief Event listener impl for (AAMP_EVENT_MANIFEST_REFRESH_NOTIFY) AAMP event
 */
class AAMP_JSListener_DashManifestRefreshNotify: public AAMP_JSListener
{
public:

	/**
     * @brief AAMP_JSListener_DashManifestRefreshNotify Constructor
     * @param[in] aamp instance of AAMP_JS
     * @param[in] type event type
     * @param[in] jsCallback callback to be registered as listener
     */
    AAMP_JSListener_DashManifestRefreshNotify(AAMP_JS* aamp, AAMPEventType type, JSObjectRef jsCallback) : AAMP_JSListener(aamp, type, jsCallback)
    {
    }
	...
	void setEventProperties(const AAMPEventPtr& e, JSContextRef context, JSObjectRef eventObj)
	{
		...
		JSObjectSetProperty(context, eventObj, prop, JSValueMakeNumber(context, evt->getManifestPublishedTime()), kJSPropertyAttributeReadOnly, NULL);
		JSStringRelease(prop);
